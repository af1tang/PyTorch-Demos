...
class MyDilatedConv2d(MyConv2d):
    ...
    def forward(self, input):
        ############### YOUR CODE GOES HERE ############### 
        '''dilation operation does: m = n + (n-1)*d
        - where n is the original kernel size (i.e. 3x3).
        - d is the dilation factor.
        - m is the dilated receptive field.
        '''
        factor = self.kernel_size + self.dilation * (self.kernel_size - 1)
        self.weight.data = F.upsample(input = self.weight, size = factor )
        mask = np.zeros(self.weight.shape)
        mask[:, :, ::self.dilation+1, ::self.dilation+1] = 1
        self.mask = torch.from_numpy(mask).float()
        self.weight.data.mul_(self.mask)
        #self.weight.data = self.weight.data * self.mask
        return F.conv2d(input, self.weight, self.bias, padding=factor//2)
        ###################################################
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.out2 = self.downconv2(self.out1)
        self.out3 = self.rfconv(self.out2)
        self.out4 = torch.cat([self.out2,self.out3], dim=1)
        self.out5 = self.upconv1(self.out4)
        self.out6 = torch.cat([self.out1, self.out5], dim=1)
        self.out7 = self.upconv2(self.out6)
        self.out8= torch.cat([x, self.out7], dim = 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default="weights/dunet_k3_f32.pkl",
    ...
    parser.add_argument('-m', '--model', choices=["CNN", "UNet", "DUNet"], default='DUNet',
                        help="Model to run")
