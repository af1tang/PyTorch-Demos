...
class WordEmbedding:
    ...
    def backward(self, grad_output):
        # YOUR CODE HERE
        grad = torch.zeros(self.vocab_size, self.embedding_dim)
        grad2 = torch.zeros(self.input.size(0), self.vocab_size)
        #note: this is not a for-loop through samples
        #iterate on word positions (like forward pass)
        for i in range(self.input.size(1)):
            self.get_indicator_matrix(self.input[:, i].long())
            self.jacobian = self.indicator_matrix
            grad += \
            torch.mm(self.jacobian.transpose(0,1), 
                     grad_output[:, i*self.embedding_dim : (i+1)*self.embedding_dim])
            grad2 += \
            torch.mm(grad_output[:, i*self.embedding_dim : (i+1)*self.embedding_dim], 
                                 self.weight.transpose(0,1))
        #average the gradients among the 3 input words for all samples
        self.grad_weight = torch.div(grad, self.input.size(1))
        grad_output = torch.div(grad2, self.input.size(1))
        #self.bias = torch.zeros(0)
        return grad_output
...
class Linear:
    ...
    def backward(self, grad_output):
        # YOUR CODE HERE
        self.jacobian = self.weight.transpose(0,1)
        self.grad_weight = torch.mm(self.input.transpose(0,1), grad_output)
        self.grad_bias = torch.mean(grad_output, 0)
        grad_output = torch.mm(grad_output, self.jacobian)
        return grad_output
...
class Sigmoid():
    ...
    def backward(self, grad_output):
        # YOUR CODE HERE
        self.jacobian = torch.mul(self.input, torch.ones(self.input.size()) - self.input)
        grad_output = torch.mul(grad_output, self.jacobian)
        return grad_output
...
class CrossEntropy():
    ...
    def backward(self, grad_output):
        # YOUR CODE HERE
        grad_output = self.prob - self.get_indicator_matrix(self.y)
        return grad_output
